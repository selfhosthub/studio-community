...
Manage your organization: team access, secrets/credentials and webhooks.
...
Org admins can promote users to Admin. The Super Admin role is reserved for the platform operator and is not assignable from the org admin UI.
...
Admins can reveal a credential's value, edit it, or delete it from `/secrets`. Regular users cannot - they can pick an existing credential when configuring a workflow step, but cannot view its value or modify it.
...
- **Form** - if any step parameter uses form mapping, or if an AI prompt has variables, Studio presents a form before the workflow runs.
...
Webhooks let outside systems trigger a workflow. Only admins can create or rotate webhooks.

### Create a webhook

Webhooks are configured per workflow inside the workflow editor:

1. Open the workflow editor (`/workflows/{id}/edit`)
2. Set the trigger type to **Webhook**
3. Pick an authentication type (see below)
4. Save the workflow
...
2. Click **Regenerate secret** in the trigger settings
...
Admins can delete any file in the organization from the Files page (`/files`). Regular users can only delete files attached to instances they created or files they uploaded.

Use admin-level deletion when:
- A user has left the org and you need to clean up their outputs
- An instance was retained for audit but its files are no longer needed
- A file was uploaded to the wrong workflow

Deletion removes the file and its thumbnails. It cannot be undone.
