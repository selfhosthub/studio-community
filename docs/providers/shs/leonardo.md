...
> The callback key and URL are generated by Studio for your convenience — Studio **cannot** set them on Leonardo's side, so you must paste the exact values into the Leonardo dashboard yourself. Regenerating either value after setup requires repeating the Leonardo handshake with the new value; the form warns before it lets you regenerate an existing value.
