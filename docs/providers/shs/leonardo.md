...
- **High Resolution** - Generate at up to 2048x2048 with the highResolution flag
...
## Completion Mode: Poll vs. Webhook

A Leonardo generation step can wait for its result in one of two ways, chosen per step in the flow editor (**Service Parameters → Completion Mode**):

- **Poll (default)**: Studio submits the generation and polls Leonardo until it completes. No extra setup. Use this unless you have a reason not to.
- **Webhook**: Studio submits the generation, releases the worker, and waits for Leonardo to call back when the result is ready. Better for long generations and high volume, but requires one-time setup on the credential.

### Webhook setup (one-time, per credential)

Leonardo binds the callback URL and callback key to a **production API key**, and that binding is **immutable**, so changing it means creating a new Leonardo production API key. Studio mirrors this: the callback URL and key live on the **credential**, and one credential serves every webhook step that uses it (callbacks are routed to the right step by the generation id in the payload).

On the credential form (**Providers > Leonardo AI > Credentials > Add/Edit Credential**):

1. Click **Generate** next to **Webhook Callback API Key** → Studio produces a strong random value.
2. Click **Generate** next to **Webhook Callback URL** → Studio produces the stable callback URL for this credential.
3. In the Leonardo dashboard, create a **production API key**, setting its webhook callback to the **URL** and the bearer key to the **callback API key** from step 1 (copy them with the buttons on the form).
4. Paste Leonardo's returned `api_key` into the **API Key** field, then **Save**. The credential now holds the api key + the callback key + the callback URL token.

> The callback key and URL are generated by Studio for your convenience, but Studio **cannot** set them on Leonardo's side, so you must paste the exact values into the Leonardo dashboard yourself. Regenerating either value after setup requires repeating the Leonardo handshake with the new value; the form warns before it lets you regenerate an existing value.

A credential used only in poll mode never needs these fields. Two Studio credentials may hold the same Leonardo `api_key` (e.g. one poll-only, one webhook-capable); this is supported.
...
| **dimensions** | Output resolution preset (512x512 through 2048x2048, plus 4:3, 3:4, 16:9 and 9:16 buckets) |
