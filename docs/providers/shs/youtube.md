...
| Service | Description |
|---------|-------------|
| **Upload Video** | Upload a video file to YouTube with its metadata |

One step does the whole upload. Give it the metadata and a **Video File URL**
(`body_source_url`) and it opens the resumable session, streams the file and
returns the created video.

```
Step 1: Upload Video
  → title, description, privacyStatus, tags, categoryId
  → body_source_url: the video file to send
  → Returns status_code and response_body.id, .snippet, .status
```

The step runs on the `transfer_jobs` queue, so the transfer worker streams the
file rather than the general worker holding it in memory.
...
| **400 Bad Request** `uploadType` | Upload session parameters missing | Verify the service endpoint still carries `uploadType=resumable` and `part` |
...
- Confirm `body_source_url` resolves to a file the transfer worker can reach
- A file still being written by an earlier step uploads truncated; wait for that step to finish
...
Step 3: Upload Video (youtube_upload_video)
  → Parameters:
    - title: "My Video"
    - description: "Generated by Studio"
    - privacyStatus: "private"
    - body_source_url: {{ step2.storage_url }}
  → Outputs: status_code, response_body.id, .snippet, .status
```

### Key Configuration Details

**Upload Video (Step 3):**
- Endpoint: `https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status,recordingDetails`
- Method: POST
- Metadata goes in the request body as `snippet`, `status` and `recordingDetails`
- `body_source_url` names the file; the step streams it into the session it just opened
